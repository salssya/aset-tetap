...
/**
 * Konversi Excel serial date number (mis. 46053) ke string tanggal 'm/d/Y'
 * yang sudah dikenali oleh date_expr() di dasbor. Kalau nilainya bukan angka
 * (mis. sudah berupa teks tanggal dari CSV), dibiarkan apa adanya.
 * Rentang 3653–100000 -- setara tahun ~1910 s.d. ~2173 -- dipakai sebagai
 * penanda "ini kemungkinan besar serial Excel", sama seperti excel_serial_ke_tanggal_dat().
 */
...
function saveDataPenyusutanToDatabase($con, $importedData) {
    ...
    $create_table_sql = "CREATE TABLE IF NOT EXISTS import_penyusutan (
        ...
        updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        imported_by VARCHAR(20),
        UNIQUE KEY uk_penyusutan_row (cost_center, asset, asset_subnumber, account, posting_date)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4";
    ...
    // Migrasi AMAN untuk tabel lama yang belum punya UNIQUE KEY ini -- tambah index kalau belum ada,
    // TIDAK PERNAH truncate/hapus data. Ini yang menggantikan TRUNCATE TABLE sebelumnya, supaya
    // upload baru tidak lagi menghapus seluruh data lama.
    ...
    if ($idxResPs && mysqli_num_rows($idxResPs) === 0) {
        mysqli_query($con, "ALTER TABLE import_penyusutan ADD UNIQUE KEY uk_penyusutan_row (cost_center, asset, asset_subnumber, account, posting_date)");
    }
        ...
        foreach ($importedData as $row_index => $row) {
            ...
            try {
                if (mysqli_query($con, $insert_sql)) {
                    ...
                    $error = mysqli_error($con);
                    if (strpos($error, 'Duplicate entry') !== false) {
                        // Baris ini sudah pernah diimport sebelumnya -> dilewati (skip), bukan error
                        $skipped_duplicate++;
                    } else {
                        $failed_rows[] = "Baris " . ($row_index + 2) . ": " . $error;
                    }
                ...
                // Kalau mysqli di-set exception-mode, error Duplicate entry dilempar sebagai
                // exception, bukan return false -- ditangkap per-baris supaya baris lain tetap lanjut.
                $msgRow = $eRow->getMessage();
                if (strpos($msgRow, 'Duplicate entry') !== false) {
                    $skipped_duplicate++;
                } else {
    ...
    if ($skipped_duplicate > 0) {
        error_log("Import Penyusutan: $skipped_duplicate baris dilewati karena sudah ada (duplikat).");
    }
        ...
        <div class="sidebar-wrapper">
          <nav class="mt-2">
            ...
            $iconMap = [
                ...
                'Data Penyusutan'                 => 'bi bi-table',
                'Dasbor Penyusutan'                => 'bi bi-speedometer2',
      ...
      <main class="app-main">
        ...
        <div class="app-content-header">
          ...
          <div class="container-fluid">
            <!--begin::Row-->
            <div class="row">
              <div class="col-sm-6"><h3 class="mb-0">Import Data</h3></div>
              <div class="col-sm-6">
          ...
          <div class="container-fluid">
            ...
            <div class="row">
              ...
              <div class="col-lg-6">
                <div class="card card-primary card-outline mb-4 h-100">
                  <!--begin::Header-->
                  <div class="card-header"><div class="card-title">Import Data Penyusutan</div></div>
                  <!--end::Header-->
                  <!--begin::Form-->
                  <form method="POST" enctype="multipart/form-data">
                    <!--begin::Body-->
                    <div class="card-body">
                      ...
                      <div class="mb-3">
                        ...
                        <small class="form-text text-muted">
                          ...
                          Kolom: Cost Center, Asset, Asset Subnumber, Account, Posting Date, Amount in Local Currency, Profit Center, Text
                        </small>
                      </div>
                    </div>
                    <!--end::Body-->
